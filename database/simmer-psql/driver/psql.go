...
func init() {
	database.RegisterFromInit("psql", &PostgresDriver{})
}

// Assemble is more useful for calling into the library so you don't
// have to instantiate an empty type.
func Assemble(config database.ConfigMap) (dbinfo *database.DBInfo, err error) {
...
func (p *PostgresDriver) Assemble(config database.ConfigMap) (dbinfo *database.DBInfo, err error) {
	defer func() {
	...
	user := config.MustString(database.ConfigUser)
	pass, _ := config.String(database.ConfigPass)
	dbname := config.MustString(database.ConfigDBName)
	host := config.MustString(database.ConfigHost)
	port := config.DefaultInt(database.ConfigPort, 5432)
	sslmode := config.DefaultString(database.ConfigSSLMode, "require")
	schema := config.DefaultString(database.ConfigSchema, "public")
	whitelist, _ := config.StringSlice(database.ConfigWhitelist)
	blacklist, _ := config.StringSlice(database.ConfigBlacklist)
	...
	if err != nil {
		return nil, errors.Wrap(err, "simmer-psql failed to connect to database")
	}
	...
	dbinfo.Tables, err = database.Tables(p, schema, whitelist, blacklist)
	if err != nil {
		return nil, err
	}
...
func (p *PostgresDriver) TableNames(schema string, whitelist, blacklist []string) ([]string, error) {
	...
	if len(whitelist) > 0 {
		tables := database.TablesFromList(whitelist)
		if len(tables) > 0 {
		...
		tables := database.TablesFromList(blacklist)
		if len(tables) > 0 {
...
func (p *PostgresDriver) Columns(schema, tableName string, whitelist, blacklist []string) (database.ColumnList, error) {
	...
	if len(whitelist) > 0 {
		cols := database.ColumnsFromList(whitelist, tableName)
		if len(cols) > 0 {
		...
		cols := database.ColumnsFromList(blacklist, tableName)
		if len(cols) > 0 {
...
func (p *PostgresDriver) PrimaryKeyInfo(schema, tableName string) (*database.PrimaryKey, error) {
	pkey := &database.PrimaryKey{}
	var err error
...
// Constraints looks up the constraints for a table.
func (p *PostgresDriver) Constraints(schema, tableName string) (database.ConstraintList, error) {
	var cons []database.Constraint
	var err error
	...
	query := `
		select tc.constraint_name,
			   ...
			   (case
					when c.character_maximum_length != 0
			   ...
			   exists(
					   ...
					   where sequence_name = CONCAT(c.table_name, '_', c.column_name, '_seq')
				   )                 as is_sequence,
			   c.is_nullable = 'YES' as is_nullable,
			   c.column_default
		from information_schema.table_constraints as tc
	...
	var rows *sql.Rows
	if rows, err = p.conn.Query(query, schema, tableName); err != nil {
		return nil, err
	}
	defer rows.Close()

	for rows.Next() {
		var constraintName, columnName, columnType, columnFullType, UDTName, constraintType string
		var isNullable, isSequence bool
		...
		c := p.TranslateColumnType(database.Column{Nullable: isNullable, DBType: columnType})
		con := database.Constraint{
			Name:           constraintName,
			ColumnName:     columnName,
			Nullable:       isNullable,
			ConstraintType: constraintType,
			Type:           c.Type,
			Sequenced:      isSequence,
...
func (p *PostgresDriver) ForeignKeyInfo(schema, tableName string) ([]database.ForeignKey, error) {
	var fkeys []database.ForeignKey
...
// as a Column object.
func (p *PostgresDriver) TranslateColumnType(c database.Column) database.Column {
	if c.Nullable {
		switch c.DBType {
...
// getArrayType returns the correct simmer.Array type for each database type
func getArrayType(c database.Column) (string, string) {
...
func (p PostgresDriver) Imports() (importers.Collection, error) {
	...
	col.BasedOnType = importers.Map{
		...
		"types.JSON": {
			ThirdParty: importers.List{`"github.com/randallmlough/simmer/types"`},
		},
		"types.Decimal": {
			ThirdParty: importers.List{`"github.com/randallmlough/simmer/types"`},
		},
		...
		"pgeo.Point": {
			ThirdParty: importers.List{`"github.com/randallmlough/simmer/types/pgeo"`},
		},
		"pgeo.Line": {
			ThirdParty: importers.List{`"github.com/randallmlough/simmer/types/pgeo"`},
		},
