...
type Interface interface {
	...
	Assemble(config ConfigMap) (*DBInfo, error)
...
type Constructor interface {
	...
	Columns(schema, tableName string, whitelist, blacklist []string) (ColumnList, error)
	PrimaryKeyInfo(schema, tableName string) (*PrimaryKey, error)
	Constraints(schema, tableName string) (ConstraintList, error)
...
func Tables(c Constructor, schema string, whitelist, blacklist []string) ([]Table, error) {
	...
	for _, name := range names {
		...
		if t.Constraints, err = c.Constraints(schema, name); err != nil {
			return nil, errors.Wrapf(err, "unable to fetch table constraints (%s)", name)
		}
