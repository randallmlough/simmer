...
func test{{$alias.UpPlural}}Find(t *testing.T) {
	...
	{{$alias.DownSingular}}Found, err := Find{{$alias.UpSingular}}({{if not $data.NoContext}}ctx, {{end -}} tx, {{$model.Table.PKey.Columns | stringMap (aliasCols $alias) | prefixStringSlice (printf "%s." "o") | join ", "}})
	if err != nil {
		t.Error(err)
	}
